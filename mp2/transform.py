...
def transformToMaze(alien, goals, walls, window,granularity):
    ...
    inputMap = [[[' ' for k in range(3)] for j in range(cols)] for i in range(rows)]
    initConfig = (alien.get_centroid()[0], alien.get_centroid()[1], alien.get_shape()) #[x, y, shape]
    initIdx = configToIdx(alien.get_config(), [0, 0 ,0], granularity, alien)


    for i in range(0, rows):
        for j in range(0, cols):
            idx = (i, j, 0)
            currConfig = idxToConfig(idx, [0, 0, 0], granularity, alien)
            alien.set_alien_config(currConfig)

            setShapeLevel(inputMap, alien, walls, window, granularity, goals)
            alien.set_alien_shape('Ball')
            setShapeLevel(inputMap, alien, walls, window, granularity, goals)
            alien.set_alien_shape('Vertical')
            setShapeLevel(inputMap, alien, walls, window, granularity, goals)
    
    alien.set_alien_config(initConfig)
    inputMap[initIdx[0]][initIdx[1]][initIdx[2]] = START_CHAR

    theMaze = Maze(inputMap, alien, granularity, [0, 0, 0], None)
    return theMaze  



def setShapeLevel(inputMap, alien, walls, window, granularity, goals):
    idx = configToIdx(alien.get_config(), [0, 0, 0], granularity, alien)
    if does_alien_touch_wall(alien, walls,granularity) or not is_alien_within_window(alien, window,granularity):
        inputMap[idx[0]][idx[1]][idx[2]] = WALL_CHAR
    elif does_alien_touch_goal(alien, goals):
        inputMap[idx[0]][idx[1]][idx[2]] = OBJECTIVE_CHAR
