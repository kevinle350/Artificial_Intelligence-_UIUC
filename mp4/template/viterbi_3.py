...
def viterbi_3(train, test):
    ...
    '''
    LAPLACE = 0.00000001
    SMOOTHING_HAPAX = 0.001
    SUFFIX = ['ing', 'ed', 'ise', 'en', 'ate', 'tion', 'or', 'ance', 'ence', 'sion', 'ity', 'ship', 'ly', 'er', 'able', 'less', 'ment', 'ive', 'ful', 'al', 'cy', 'ness', 'ous', 'ent', 'ant']
    #print(train[0])
    #print(test[1])
    sol = []
        ...
        for i in range(len(sentence)-1):    
            ...
            for suffix in SUFFIX:
                if w.endswith(suffix):
                    curSuffix = suffix
            if curSuffix not in suffixTagCount:
                suffixTagCount[curSuffix] = {}
            if t not in suffixTagCount[curSuffix]:
                suffixTagCount[curSuffix][t] = 0
            suffixTagCount[curSuffix][t] += 1
            ...
            if curWord in onceWord:
                ...
                for suffix in SUFFIX:
                    if curWord.endswith(suffix):
                        isSuffix = 1
                        if suffix not in hapaxCount:
                            hapaxCount[suffix] = {}
                        if t not in hapaxCount[suffix]:
                            hapaxCount[suffix][t] = 0
                        hapaxCount[suffix][t] += 1
                if not isSuffix:
                    if t not in hapaxCount['unknown']:
                        hapaxCount['unknown'][t] = 0
                    hapaxCount['unknown'][t] += 1
                        
    # with open('result.json', 'w') as fp:
    #     json.dump(hapaxCount, fp)
            ...
            if i == 0:
                for j in range(len(tagList)):
                    ...
                    for suffix in SUFFIX:
                        if w.endswith(suffix):
                            curSuffix = suffix
                    curN = sum(hapaxCount[curSuffix].values())
                    curV = len(hapaxCount[curSuffix])

                    if t not in hapaxCount[curSuffix]:
                        hapaxProb = (SMOOTHING_HAPAX/(curN+SMOOTHING_HAPAX*(curV)+1))
                    else:
                        hapaxProb = ((hapaxCount[curSuffix][t]+SMOOTHING_HAPAX)/(curN+SMOOTHING_HAPAX*(curV)+1))
