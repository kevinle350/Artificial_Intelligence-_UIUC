...
class NeuralNet(nn.Module):
    def __init__(self, lrate, loss_fn, in_size, out_size):
        ...
        self.net1 = nn.Linear(in_size, 32)
        self.net3 = nn.Linear(32, out_size)
...
def fit(train_set,train_labels,dev_set,epochs,batch_size=100):
    ...
    for x in range(epochs):
        ...
        while curr < len(stan_train):
            image = stan_train[curr:min(curr+batch_size, len(stan_train))]
            label = train_labels[curr:min(curr+batch_size, len(stan_train))]
            loss.append(net.step(image, label))
            curr += batch_size
        a = np.mean(loss)
        losses.append(a)

    yhats = np.zeros(len(dev_set))
    pred = net.forward(stan_dev)

    for x in range(len(pred)):
        yhats[x] = torch.argmax(pred[x])
    
    return losses, yhats.astype(np.int), net
